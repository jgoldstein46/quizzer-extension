...
export class QuizController {
  ...
  async generateQuiz(request: QuizGenerationRequest): Promise<QuizGenerationResponse> {
    ...
    try {
      this.emitEvent('started', 'Starting quiz generation');
      ...
      // Process content
      this.emitEvent('processing_content', 'Preprocessing article content');
      ...
      // Call API
      this.emitEvent('calling_api', 'Generating quiz questions using Claude API');
      ...
      // Parse response
      this.emitEvent('parsing_response', 'Processing quiz questions');
      ...
      // Store quiz
      this.emitEvent('storing_quiz', 'Saving quiz to storage');
      
      // Ensure metadata has required fields
      const quizWithRequiredMetadata = {
        ...quiz,
        metadata: {
          ...(quiz.metadata || {}),
          generatedAt: quiz.metadata?.generatedAt || new Date().toISOString()
        }
      };
      ...
      this.emitEvent('complete', 'Quiz generation complete', {
